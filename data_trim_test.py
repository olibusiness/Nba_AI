...
def convert_ppg(career_PTS, career_G):
    ppg = career_PTS / career_G
    return ppg
...
# merged_df["PPG"] = convert_ppg(merged_df["career_PTS"], merged_df["career_G"])


master_data = merged_df.groupby("player_id").agg(
    {
        "name": "first",
        "salary": "last",
        "player_id": "first",
...
master_data.to_csv("master_data_test.csv", index=False)
